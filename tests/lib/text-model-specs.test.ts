...
describe('text-model-specs P2', () => {
  it('ships specs for each of the four vercel-ai providers', () => {
    const minimax = getTextModelSpecsByProvider('minimax');
    const openai = getTextModelSpecsByProvider('openai');
    const anthropic = getTextModelSpecsByProvider('anthropic');
    const openrouter = getTextModelSpecsByProvider('openrouter');
    ...
    expect(anthropic.length).toBeGreaterThanOrEqual(1); // claude-3-haiku
    expect(openrouter.length).toBeGreaterThanOrEqual(1); // openai/gpt-4o-mini
  ...
  it('mode override is consistent across models with the shared profile', () => {
    ...
    for (const modelId of ['MiniMax-M2.5', 'MiniMax-M2.7', 'gpt-4o-mini', 'claude-3-haiku-20240307']) {
  ...
  it('getAllTextModelSpecs returns at least 6 specs', () => {
